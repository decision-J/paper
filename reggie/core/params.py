...
class Parameterized(object):
    ...
    def hyper(self):
        ...
        iterable = (t.get_transform(x) for (x, t) in
                    zip(self.__params, self.__transforms))
    ...
    def hyper(self, value):
        ...
        z = zip(value, self.__domains, self.__priors, self.__transforms)
    ...
    def _wrap_gradient(self, gradient):
        ...
        iterable = (t.get_gradfactor(x)
                    for (x, t) in zip(self.__params, self.__transforms))
    ...
    def get_logprior(self, grad=False):
        ...
        if self.__params is None:
            raise AttributeError('hyperparameters of a sub-model cannot be '
                                 'accessed')
        if grad:
            zipped = zip(self.__params, self.__priors, self.__transforms)
            logprior = 0.0
            ...
            zipped = zip(self.__params, self.__priors)
    ...
    def get_logjacobian(self):
        ...
        return sum(np.log(t.get_gradfactor(d))
                   for (d, t) in zip(self.__params, self.__transforms))
